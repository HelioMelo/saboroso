...
router.get("/reservations/chart", function(req, res, next) {
    // Verificar se req.query está definido
    if (!req.query) {
        return res.status(400).json({ error: "Parâmetros de consulta ausentes" });
    }

    // Formatar as datas de início e fim usando Moment.js
    req.query.start = req.query.start ? moment(req.query.start).format('YYYY-MM-DD') : moment().subtract(1, 'year').format('YYYY-MM-DD');
    req.query.end = req.query.end ? moment(req.query.end).format('YYYY-MM-DD') : moment().format('YYYY-MM-DD');

    // Chamar a função chart para obter os dados do gráfico
    reservations.chart(req.query)
        .then(chartData => {
            // Enviar os dados do gráfico como resposta JSON
            res.json(chartData);
        })
        .catch(err => {
            // Lidar com erros enviando uma resposta de erro
